...
const HomePage: React.FC = () => {
  ...
  return (
    <div className="relative overflow-x-hidden">
      ...
      <section className="py-16 sm:py-32 px-5 sm:px-12 max-w-7xl mx-auto relative z-10">
      ...
      <section className="py-16 sm:py-36 px-5 sm:px-12 max-w-6xl mx-auto text-center relative">
        <motion.div
        ...
        >
          ...
          <Sparkles className="mx-auto mb-8 sm:mb-10 text-emerald-500 opacity-40" size={36} />

          <h3 className="text-3xl sm:text-5xl md:text-6xl font-black dark:text-white text-morandi-slate mb-8 sm:mb-10 tracking-tight sm:tracking-tighter leading-[1.12] text-glow selection:bg-white selection:text-black">
            從維運現場出發<br /><span className="opacity-35 italic">打造穩定的基礎架構</span>
          </h3>

          <p className="dark:text-slate-400 text-morandi-stone text-base sm:text-xl md:text-2xl leading-relaxed max-w-2xl mx-auto mb-10 sm:mb-12 font-light tracking-normal sm:tracking-tight">
...
const StatsCard: React.FC<{ icon: React.ReactNode; title: string; desc: string; detail: string; className?: string }> = ({ icon, title, desc, detail, className = '' }) => (
  <motion.div
    ...
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true }}
    className={`glass-panel p-8 sm:p-12 lg:p-16 flex flex-col items-center text-center group dark:bg-white/[0.03] bg-white/60 transition-all duration-500 shadow-xl rounded-3xl sm:rounded-[3rem] ${className}`}
  >
    ...
    <h4 className="dark:text-slate-500 text-morandi-stone font-black text-[10px] uppercase tracking-[0.35em] sm:tracking-[0.5em] mb-6 sm:mb-8">{title}</h4>
    <p className="dark:text-white text-morandi-slate text-3xl sm:text-5xl lg:text-6xl font-[900] mb-4 tracking-tight sm:tracking-tighter group-hover:text-glow transition-all">{desc}</p>
    <p className="dark:text-slate-400 text-morandi-stone/60 text-[10px] font-bold uppercase tracking-[0.24em] sm:tracking-[0.3em]">{detail}</p>
  </motion.div>
);
